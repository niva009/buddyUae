...
import { SEND_OTP, VERIFY_OTP, basicRequest, basicXFormRequest } from "../../../components/api/index";
...
export default function VerifyOtpScreen() {
  ...
  const [otp, setOtp] = useState(new Array(OTP_LENGTH).fill(""));  // Add this line to initialize otp state

  const inputRefs = useRef([]);
  inputRefs.current = new Array(OTP_LENGTH).fill().map((_, index) => inputRefs.current[index] || React.createRef());
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      formData.append("otp", otp.join(""));  // Join OTP digits before sending
              ...
              <div className="w-[92%] lg:w-full relative max-h-[630px] min-h-[620px] grid lg:grid-cols-2 max-w-5xl h-full items-center transform overflow-hidden rounded bg-white shadow-xl transition-all">
                ...
                <div className="flex">
                  <div className="flex flex-col h-full px-5 lg:px-20 items-start">
                    ...
                    <button
                      disabled={otp.every((digit) => digit === "") && waitingTime === 0}
                      onClick={verifyOtp}
