...
import { newRequest, ORDER_DETAILS } from "../../../components/api/index";
import useCurrencyFormatter from "../../../utils/useCurrency";
import { useParams } from "next/navigation";

export default function OrderDetails() {
  const { id } = useParams(); // Next.js App Router params
  ...
  const { data: orderDetails, isLoading } = useQuery({
    ...
    queryFn: () =>
      newRequest
        .get(ORDER_DETAILS, {
          params: { customer_id: user, order_id: id },
        })
        .then((res) => res.data),
    enabled: !!id && !!user,
  });
                    ...
                    <div className="flex items-center gap-3">
                      <div className="flex w-24 p-1 h-28 border border-black/15 rounded">
                        <img
                          ...
                          src={`${process.env.NEXT_PUBLIC_IMG_URL}${i?.thumbnail_img_link}}`}
                          alt={i?.name}
                        />
                      </div>
