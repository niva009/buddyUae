...
export default function ChooseAddress() {
  ...
  const { user, addClientSecret } = useUserStore((state) => ({
    user: state.user,
    addClientSecret: state.addClientSecret,
  }));
  ...
  const [minAmmountForDiscountAmount, setMinAmmountForDiscountAmount] =
    useState(0);
  const TAX_RATE = 0.05; // Example tax rate of 5%
  ...
  const { data: address } = useQuery({
    ...
    queryFn: () =>
      newRequest
        .get(ADDRESSES, { params: { customer_id: user?.id } })
        .then((res) => res.data),
    enabled: !!user?.id,
  });
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="text-[.85rem] max-w-[17rem] font-medium">
                          {i?.pincode}, {i?.flat_building}, {i?.address},{" "}
                          {i?.city}, {i?.state},{i?.locality},{i?.land_mark}
                        </div>
